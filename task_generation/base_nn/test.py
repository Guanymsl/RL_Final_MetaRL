...
class CFRDataset(Dataset):
    def __init__(self, path="../cfr_bc_dataset.npz"):
        data = np.load(path)
...
# Load dataset
# ----------------------
dataset = CFRDataset("../cfr_bc_dataset.npz")
loader = DataLoader(dataset, batch_size=4096, shuffle=True, drop_last=True)

print("\n=== Dataset Integrity Check ===")
print("probs min:", dataset.probs.min())
print("probs max:", dataset.probs.max())
print("probs row sums (first 10):", dataset.probs[:10].sum(axis=1))
print("Any NaN in probs:", np.isnan(dataset.probs).any())
print("Any NaN in obs:", np.isnan(dataset.obs).any())
print("Any negative probs:", np.any(dataset.probs < 0))
print("Any probs > 1:", np.any(dataset.probs > 1))
...
# Save Model
# ----------------------
torch.save(model.state_dict(), "cfr_bc_policy.pt")
print("Saved model to cfr_bc_policy.pt")
